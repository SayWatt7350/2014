...
task main () {
	...
	for(int x=5;x>0;x--)//give user time to calibrate
	{
		nxtDisplayCenteredTextLine(1,"Calibration in:");
	...
	startCalibration();
	//StartTask(showPulse);
	for(int quadrant=1;quadrant<5;quadrant++)//Tell user time for each quadrant
	{
		nxtDisplayCenteredTextLine(1,"Quadrant %d Time:", quadrant);

		for(int time=5;time>0;time--)
		{
			nxtDisplayCenteredBigTextLine(3,"%d",time);
			wait1Msec(1000);
		}
	}
	eraseDisplay();
